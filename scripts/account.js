...
Api.AccountButton.addEventListener("click", async () => {
    ...
    Submit.addEventListener("click", async () => {
        ...
        const User = { Username, Password, Followers: [], ProfileImage: "../images/Default.svg" };
...
(async () => {
    if (localStorage.getItem("User")) {
        ...
        Api.AccountButton.innerHTML = `<img class="ProfileImageLabel" src="${Documents[0].ProfileImage}">`;
        if (Documents.length > 0) Api.UsernameLabels.forEach(Label => Label.innerHTML = Documents[0].Username);
        else {
            localStorage.removeItem("User");
            location.reload();
        }
    } else Api.UsernameLabels.forEach(Label => Label.innerHTML = "Sign In")
})();