const Storage = new Datastore("Users");
AccountButton.addEventListener("click", async () => {
    ...
    Submit.addEventListener("click", async () => {
        ...
        let User = { Username, Password, Followers: [], ProfileImage: "../images/Default.svg" };
...
(async () => {
    if (localStorage.getItem("User")) {
        ...
        AccountButton.innerHTML = `<img class="ProfileImageLabel" src="${Documents[0].ProfileImage}">`;
        if (Documents.length > 0) UsernameLabels.forEach(Label => Label.innerHTML = Documents[0].Username);
        else {
            localStorage.removeItem("User");
            location.reload();
        }
    } else UsernameLabels.forEach(Label => Label.innerHTML = "Sign In")
})();