Topbar.querySelectorAll("input").forEach(Input => Input.addEventListener("focus", () => Content.style.opacity = "0.5"));
Topbar.querySelectorAll("input").forEach(Input => Input.addEventListener("blur", () => Content.style.opacity = ""));

const Login = localStorage.getItem("User");
const User = JSON.parse(localStorage.getItem("User"));
const PostStorage = new Datastore("Posts");

const FetchPosts = async (Algorithm) => {
    await PostStorage.GetDocuments().then((Documents) => {
        Array.from(Posts.children).forEach((Child) => Child.classList.contains("Post") ? Child.remove() : "");
        Documents.forEach(async (Document) => {
            const AuthorDocuments = await new Datastore("Users").GetDocumentsByField("Username", Document.Author);
            ...
            Post.innerHTML = `
                <div>
                    <div>
                        ...
                        <img style="position: absolute; right: 8px; cursor: pointer; border: none; display: ${Login ? Document.Author === User.Username ? "" : "none" : "none"}};" src="../images/Dots.svg">
                    </div>
            ...
            if (Login && Document.Author === User.Username) FollowButton.style.display = "none";
    
            if (Login && Followers.includes(User.Username)) {
                FollowButton.innerHTML = "Unfollow";
                FollowButton.setAttribute("active", "");
                if (Algorithm === "Followed") Post.style.order = Post.style.order - 1;
            } else {
                FollowButton.innerHTML = "Follow";
                FollowButton.removeAttribute("active");
            }
    
            if (Algorithm === "Hot") Post.style.order = Post.style.order - Comments.length - Upvotes.length + Downvotes.length;
            else if (Algorithm === "New") Post.style.order = Post.style.order - Document.Timestamp;
            ...
            FollowButton.onclick = async () => {
                ...
                await new Datastore("Users").UpdateDocument(AuthorDocument.id, { Followers: Followers });
            ...
            const UpdateUI = () => {
                if (Login) {
                    ...
                    [
                        ...
                        Instance.style.cursor = "not-allowed";
                    });
                }
                ...
                Comments.forEach(async (Comment) => {
                    const AuthorDocuments = await new Datastore("Users").GetDocumentsByField("Username", Comment.Author);
                    const AuthorDocument = AuthorDocuments[0];
            ...
            Document.Attachments.forEach(Attachment => {
                if (Attachment.Type === "Image") AttachmentContainer.innerHTML += `<img class="Attachment" src="${Attachment.Url}">`;
                else if (Attachment.Type === "Video") AttachmentContainer.innerHTML += `<video class="Attachment" src="${Attachment.Url}"></video>`;
            });

            Array.from(AttachmentContainer.children).forEach(Attachment => Attachment.addEventListener("click", () => window.open(Attachment.getAttribute("src"))));
...
Array.from(Pagination.children).forEach(Node => {
    if (!Node.hasAttribute("href")) return;
    Node.addEventListener("click", () => {
        Array.from(Pagination.children).forEach(Node => Node.removeAttribute("selected"));
        Node.setAttribute("selected", "");

        Array.from(Content.children).forEach(Node => Node.style.display = "none");
        Content.querySelector(`[href="${Node.getAttribute("href")}"]`).style.display = "";
    });
});

ProfileImageLabels.forEach(Image => Image.src = localStorage.getItem("User") ? JSON.parse(localStorage.getItem("User")).ProfileImage : "");
...
UploadProfileImageButton.addEventListener("click", async () => {
    ...
    const Documents = await new Datastore("Users").GetDocumentsByField("Username", JSON.parse(localStorage.getItem("User")).Username);

    await new Datastore("Users").UpdateDocument(Documents[0].id, { ProfileImage: `https://github.com/kayyraa/DirectStorage/blob/main/${Path}?raw=true` });
    location.reload();
});
...
AttachmentInput.addEventListener("change", async () => {
    ...
    if (PostAttachmentContainer.querySelector("span")) PostAttachmentContainer.querySelector("span").remove();
    PostAttachmentContainer.innerHTML += `<img style="max-width: 10em; max-height: 10em;" src="https://github.com/kayyraa/DirectStorage/blob/main/${Path}?raw=true">`;
});
...
const UserDocuments = await new Datastore("Users").GetDocumentsByField("Username", localStorage.getItem("User") ? JSON.parse(localStorage.getItem("User")).Username : "");
...
FollowingLabel.innerHTML = `${FollowingSet.size} Following`;

await FetchPosts("Hot");
Array.from(Filters.children).forEach(Filter => {
    Filter.addEventListener("click", () => {
        FetchPosts(Filter.getAttribute("href"));
        Array.from(Filters.children).forEach(Filter => Filter.removeAttribute("active"));
